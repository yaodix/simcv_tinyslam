...
void ScanMatchingPLICP::ScanMatching(pcl::PointCloud<pcl::PointXYZI>::Ptr ref,pcl::PointCloud<pcl::PointXYZI>::Ptr per, const std::vector<double>& init_pose)
{
    ...
    for(int i = 0; i < per->points.size(); i++)
    {
        if (kdtree.nearestKSearch(per->points[i], K, index, distance) == K){
            ...
            ceres::LossFunction *loss_function1 = new ceres::CauchyLoss(0.2); //2.1 设定 loss function
...
void ScanMatchingPLICP::ComputeCovariance(vector<Eigen::Vector2d>& data_pi, vector<Eigen::Vector2d>& model_qi, Eigen::Vector3d& transform, Eigen::MatrixXd& ICP_COV)
{
    ...
    for (size_t s = 0; s < count; ++s )
    {
        ...
        Apn_temp << (ni.transpose()*ai_ver).dot((ni.transpose()*ai_ver)), (ni.transpose()*ai_ver)*ni.transpose(),
